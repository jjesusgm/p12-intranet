...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE inv_mobiliario SET id_marca=%s, id_resguardante=%s, id_tipo=%s, id_material=%s, id_color=%s, numero_serie=%s, numero_inv_udg=%s, descripcion=%s, id_ubicacion=%s, comentario=%s WHERE id_mobiliario=%s",
                       ...
                       GetSQLValueString($_POST['fNoInvUdg'], "text"),
...
$colname_rsMobiliario = "-1";
if (isset($_GET['id_mobiliario'])) {
  $colname_rsMobiliario = $_GET['id_mobiliario'];
}
mysql_select_db($database_conP12, $conP12);
$query_rsMobiliario = sprintf("SELECT * FROM inv_mobiliario WHERE id_mobiliario = %s", GetSQLValueString($colname_rsMobiliario, "int"));
$rsMobiliario = mysql_query($query_rsMobiliario, $conP12) or die(mysql_error());
$row_rsMobiliario = mysql_fetch_assoc($rsMobiliario);
$totalRows_rsMobiliario = mysql_num_rows($rsMobiliario);
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Inventario &gt; Mobiliario &gt; Modificar</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <h2 class="center">Modificando mobiliario</h2>
  <form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
    <table width="600" align="center" class="tabla_usuario">
      <tr>
        <th colspan="2" align="left" scope="col">Registro del mobiliario a modificar</th>
      </tr>
      <tr>
        <td align="right"><label for="fIdMobiliario">ID</label></td>
        <td><input name="fIdMobiliario" type="text" id="fIdMobiliario" value="<?php echo $row_rsMobiliario['id_mobiliario']; ?>" readonly /></td>
      </tr>
      <tr>
...
do {  
?>
          <option value="<?php echo $row_rsMarcas['id_marca']?>"<?php if (!(strcmp($row_rsMarcas['id_marca'], $row_rsMobiliario['id_marca']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsMarcas['marca']?></option>
          <?php
...
do {  
?>
          <option value="<?php echo $row_rsResguardantes['username']?>"<?php if (!(strcmp($row_rsResguardantes['username'], $row_rsMobiliario['id_resguardante']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsResguardantes['nombre_completo']?></option>
          <?php
...
do {  
?>
          <option value="<?php echo $row_rsTipos['id_tipo_mobiliario']?>"<?php if (!(strcmp($row_rsTipos['id_tipo_mobiliario'], $row_rsMobiliario['id_tipo']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsTipos['tipo_mobiliario']?></option>
          <?php
...
do {  
?>
          <option value="<?php echo $row_rsMateriales['id_material']?>"<?php if (!(strcmp($row_rsMateriales['id_material'], $row_rsMobiliario['id_material']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsMateriales['material']?></option>
          <?php
...
do {  
?>
          <option value="<?php echo $row_rsColores['id_color']?>"<?php if (!(strcmp($row_rsColores['id_color'], $row_rsMobiliario['id_color']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsColores['color']?></option>
          <?php
...
?>
      ...
      <tr>
        ...
        <td><input name="fNumeroSerie" type="text" id="fNumeroSerie" value="<?php echo $row_rsMobiliario['numero_serie']; ?>" size="25" maxlength="20" /></td>
      </tr>
      <tr>
        <td align="right"><label for="fNoInvUdg">No. inventario UdeG</label></td>
        <td><input name="fNoInvUdg" type="text" id="fNoInvUdg" value="<?php echo $row_rsMobiliario['numero_inv_udg']; ?>" size="25" maxlength="20" /></td>
      </tr>
      <tr>
        <td align="right"><label for="fDescripcion">Descripción</label></td>
        <td><input name="fDescripcion" type="text" id="fDescripcion" value="<?php echo $row_rsMobiliario['descripcion']; ?>" size="55" maxlength="100" /></td>
      </tr>
      <tr>
...
do {  
?>
          <option value="<?php echo $row_rsUbicaciones['id_ubicacion']?>"<?php if (!(strcmp($row_rsUbicaciones['id_ubicacion'], $row_rsMobiliario['id_ubicacion']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsUbicaciones['ub_nombre']?></option>
          <?php
...
?>
      ...
      <tr>
        ...
        <td><input name="fComentario" type="text" id="fComentario" value="<?php echo $row_rsMobiliario['comentario']; ?>" size="55" maxlength="100" /></td>
      </tr>
      <tr>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsMobiliario);
