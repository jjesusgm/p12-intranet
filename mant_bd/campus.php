...
$maxRows_rsCampus = 10;
$pageNum_rsCampus = 0;
if (isset($_GET['pageNum_rsCampus'])) {
  $pageNum_rsCampus = $_GET['pageNum_rsCampus'];
}
$startRow_rsCampus = $pageNum_rsCampus * $maxRows_rsCampus;

mysql_select_db($database_conP12, $conP12);
$query_rsCampus = "SELECT * FROM p12_campus ORDER BY campus ASC";
$query_limit_rsCampus = sprintf("%s LIMIT %d, %d", $query_rsCampus, $startRow_rsCampus, $maxRows_rsCampus);
$rsCampus = mysql_query($query_limit_rsCampus, $conP12) or die(mysql_error());
$row_rsCampus = mysql_fetch_assoc($rsCampus);

if (isset($_GET['totalRows_rsCampus'])) {
  $totalRows_rsCampus = $_GET['totalRows_rsCampus'];
} else {
  $all_rsCampus = mysql_query($query_rsCampus);
  $totalRows_rsCampus = mysql_num_rows($all_rsCampus);
}
$totalPages_rsCampus = ceil($totalRows_rsCampus/$maxRows_rsCampus)-1;
?>
<!DOCTYPE html>
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Mantenimiento a BD &gt; Campus</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <h2 class="center">Mostrando
registros <?php echo ($startRow_rsCampus + 1) ?> al <?php echo min($startRow_rsCampus + $maxRows_rsCampus, $totalRows_rsCampus) ?> de <?php echo $totalRows_rsCampus ?> </h2>
  <h3 class="center"><a href="campus_add.php" class="estilo3">Agrgar un campus</a></h3>
  <table width="600" align="center" class="tabla_contactos">
    ...
    <?php do { ?>
      <tr>
        <td align="center"><?php echo $row_rsCampus['id_campus']; ?></td>
        <td><?php echo $row_rsCampus['campus']; ?></td>
        <td align="center"><a href="campus_edit.php?id_campus=<?php echo $row_rsCampus['id_campus']; ?>" class="estilo3">Editar</a> | <a href="campus_delete.php?id_campus=<?php echo $row_rsCampus['id_campus']; ?>" class="estilo3">Eliminar</a></td>
      </tr>
      <?php } while ($row_rsCampus = mysql_fetch_assoc($rsCampus)); ?>
  </table><br />
</div>
<!-- InstanceEndEditable -->
<div id="div_footer">
