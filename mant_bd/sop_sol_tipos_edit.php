...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE sop_sol_tipos SET id_nivel_acceso=%s, tipo_sol=%s WHERE id_tipo_sol=%s",
                       GetSQLValueString($_POST['fIdNivelAcceso'], "int"),
                       GetSQLValueString($_POST['fTipoSol'], "text"),
...
$colname_rsSopSolTipo = "-1";
if (isset($_GET['id_tipo_sol'])) {
  $colname_rsSopSolTipo = $_GET['id_tipo_sol'];
}
mysql_select_db($database_conP12, $conP12);
$query_rsSopSolTipo = sprintf("SELECT * FROM sop_sol_tipos WHERE id_tipo_sol = %s", GetSQLValueString($colname_rsSopSolTipo, "int"));
$rsSopSolTipo = mysql_query($query_rsSopSolTipo, $conP12) or die(mysql_error());
$row_rsSopSolTipo = mysql_fetch_assoc($rsSopSolTipo);
$totalRows_rsSopSolTipo = mysql_num_rows($rsSopSolTipo);

mysql_select_db($database_conP12, $conP12);
$query_rsNivelesAcceso = "SELECT * FROM usr_niveles_acceso ORDER BY nivel_de_acceso ASC";
$rsNivelesAcceso = mysql_query($query_rsNivelesAcceso, $conP12) or die(mysql_error());
$row_rsNivelesAcceso = mysql_fetch_assoc($rsNivelesAcceso);
$totalRows_rsNivelesAcceso = mysql_num_rows($rsNivelesAcceso);
?>
<!DOCTYPE html>
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Mantenimiento a BD &gt; Soporte &gt; Tipos de solicitud &gt; Modificar</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <h2 class="center">Modificando tipo de solicitud de soporte</h2>
  <form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
    <table width="500" align="center" class="tabla_usuario">
      <tr>
        <th colspan="2" align="left" scope="col">Registro a modificar</th>
      </tr>
      <tr>
        <td align="right"><label for="fIdTipoSol">ID</label></td>
        <td><input name="fIdTipoSol" type="text" id="fIdTipoSol" value="<?php echo $row_rsSopSolTipo['id_tipo_sol']; ?>" size="5" maxlength="3" readonly /></td>
      </tr>
      <tr>
        <td align="right"><label for="fIdNivelAcceso">Perfil*</label></td>
        <td><select name="fIdNivelAcceso" id="fIdNivelAcceso">
          <option value="0" <?php if (!(strcmp(0, $row_rsSopSolTipo['id_nivel_acceso']))) {echo "selected=\"selected\"";} ?>>Todos</option>
          <?php
do {  
?>
          <option value="<?php echo $row_rsNivelesAcceso['id_nivel_acceso']?>"<?php if (!(strcmp($row_rsNivelesAcceso['id_nivel_acceso'], $row_rsSopSolTipo['id_nivel_acceso']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsNivelesAcceso['nivel_de_acceso']?></option>
          <?php
} while ($row_rsNivelesAcceso = mysql_fetch_assoc($rsNivelesAcceso));
  $rows = mysql_num_rows($rsNivelesAcceso);
  if($rows > 0) {
      mysql_data_seek($rsNivelesAcceso, 0);
	  $row_rsNivelesAcceso = mysql_fetch_assoc($rsNivelesAcceso);
  }
?>
        </select></td>
      ...
      <tr>
        <td align="right"><label for="fTipoSol">Tipo*</label></td>
        <td><input name="fTipoSol" type="text" id="fTipoSol" value="<?php echo $row_rsSopSolTipo['tipo_sol']; ?>" size="55" maxlength="50" /></td>
      </tr>
      <tr>
        <td colspan="2" align="center"><input type="submit" name="bModificar" id="bModificar" value="Modificar" />
          <a href="sop_sol_tipos.php" class="button-link">Cancelar</a></td>
      </tr>
    </table>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsSopSolTipo);

mysql_free_result($rsNivelesAcceso);
?>
