...
$maxRows_rsCiclos = 6;
$pageNum_rsCiclos = 0;
if (isset($_GET['pageNum_rsCiclos'])) {
  $pageNum_rsCiclos = $_GET['pageNum_rsCiclos'];
}
$startRow_rsCiclos = $pageNum_rsCiclos * $maxRows_rsCiclos;

mysql_select_db($database_conP12, $conP12);
$query_rsCiclos = "SELECT * FROM p12_ciclos WHERE ciclo <> 'No aplica' ORDER BY ciclo DESC";
$query_limit_rsCiclos = sprintf("%s LIMIT %d, %d", $query_rsCiclos, $startRow_rsCiclos, $maxRows_rsCiclos);
$rsCiclos = mysql_query($query_limit_rsCiclos, $conP12) or die(mysql_error());
$row_rsCiclos = mysql_fetch_assoc($rsCiclos);

if (isset($_GET['totalRows_rsCiclos'])) {
  $totalRows_rsCiclos = $_GET['totalRows_rsCiclos'];
} else {
  $all_rsCiclos = mysql_query($query_rsCiclos);
  $totalRows_rsCiclos = mysql_num_rows($all_rsCiclos);
}
$totalPages_rsCiclos = ceil($totalRows_rsCiclos/$maxRows_rsCiclos)-1;
?>
<?php
$queryString_rsCiclos = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rsCiclos") == false && 
        stristr($param, "totalRows_rsCiclos") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rsCiclos = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rsCiclos = sprintf("&totalRows_rsCiclos=%d%s", $totalRows_rsCiclos, $queryString_rsCiclos);
?>
<!DOCTYPE html>
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Mantenimiento a BD &gt; Uso general &gt; Ciclos</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <h2 class="center">Mostrando Ciclos <?php echo ($startRow_rsCiclos + 1) ?> al <?php echo min($startRow_rsCiclos + $maxRows_rsCiclos, $totalRows_rsCiclos) ?> de <?php echo $totalRows_rsCiclos ?> </h2>
  <h3 class="center"><a href="cic_add.php" class="estilo3">Agregar un ciclo</a></h3>
  <table border="0" align="center">
    <tr>
      <td>Navegación:</td>
      <td><a href="<?php printf("%s?pageNum_rsCiclos=%d%s", $currentPage, 0, $queryString_rsCiclos); ?>" class="estilo3">Primera</a> | <a href="<?php printf("%s?pageNum_rsCiclos=%d%s", $currentPage, max(0, $pageNum_rsCiclos - 1), $queryString_rsCiclos); ?>" class="estilo3">Anterior</a> | <a href="<?php printf("%s?pageNum_rsCiclos=%d%s", $currentPage, min($totalPages_rsCiclos, $pageNum_rsCiclos + 1), $queryString_rsCiclos); ?>" class="estilo3">Siguiente</a> | <a href="<?php printf("%s?pageNum_rsCiclos=%d%s", $currentPage, $totalPages_rsCiclos, $queryString_rsCiclos); ?>" class="estilo3">&Uacute;ltima</a></td>
    </tr>
  </table>
<table width="450" align="center" class="tabla_contactos">
    <tr>
    <th scope="col">ID</th>
  ...
  <?php do { ?>
    <tr>
      <td align="center"><?php echo $row_rsCiclos['id_ciclo']; ?></td>
      <td align="center"><?php echo $row_rsCiclos['ciclo']; ?></td>
      <td align="center"><a href="ciclo_edit.php" class="estilo3">Editar</a> | <a href="ciclo_delete.php" class="estilo3">Eliminar</a></td>
    </tr>
    <?php } while ($row_rsCiclos = mysql_fetch_assoc($rsCiclos)); ?>
</table><br />
</div>
<!-- InstanceEndEditable -->
<div id="div_footer">
