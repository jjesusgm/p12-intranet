...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE inv_computo_tipos_monitor SET tm_nombre=%s WHERE siglas=%s",
                       GetSQLValueString($_POST['fTMNombre'], "text"),
                       GetSQLValueString($_POST['fSiglas'], "text"));

  mysql_select_db($database_conP12, $conP12);
  $Result1 = mysql_query($updateSQL, $conP12) or die(mysql_error());

  $updateGoTo = "inv_comp_tipos_monitor.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsTipoMonitor = "-1";
if (isset($_GET['siglas'])) {
  $colname_rsTipoMonitor = $_GET['siglas'];
}
mysql_select_db($database_conP12, $conP12);
$query_rsTipoMonitor = sprintf("SELECT * FROM inv_computo_tipos_monitor WHERE siglas = %s", GetSQLValueString($colname_rsTipoMonitor, "text"));
$rsTipoMonitor = mysql_query($query_rsTipoMonitor, $conP12) or die(mysql_error());
$row_rsTipoMonitor = mysql_fetch_assoc($rsTipoMonitor);
$totalRows_rsTipoMonitor = mysql_num_rows($rsTipoMonitor);
?>
<!DOCTYPE html>
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Mantenimiento a BD &gt; Inventario &gt; Cómputo &gt; Tipos de monitor &gt; Editar</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <h2 class="center">Editando un tipo de monitor</h2>
  <form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
    <table width="500" align="center" class="tabla_usuario">
      <tr>
        <th colspan="2" align="left" scope="col">Registro a editar</th>
      </tr>
      <tr>
        <td align="right"><label for="fSiglas">Siglas</label></td>
        <td><input name="fSiglas" type="text" id="fSiglas" value="<?php echo $row_rsTipoMonitor['siglas']; ?>" size="15" maxlength="10" readonly /></td>
      </tr>
      <tr>
        <td align="right"><label for="fTMNombre">Tipo*</label></td>
        <td><input name="fTMNombre" type="text" id="fTMNombre" value="<?php echo $row_rsTipoMonitor['tm_nombre']; ?>" size="45" maxlength="40" /></td>
      </tr>
      <tr>
        <td colspan="2" align="center"><input type="submit" name="fActualizar" id="fActualizar" value="Actualizar" />
          <a href="inv_comp_tipos_monitor.php" class="button-link">Cancelar</a></td>
      </tr>
    </table>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($rsTipoMonitor);
?>
