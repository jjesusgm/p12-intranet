...
$maxRows_rsCarreras = 10;
$pageNum_rsCarreras = 0;
if (isset($_GET['pageNum_rsCarreras'])) {
  $pageNum_rsCarreras = $_GET['pageNum_rsCarreras'];
}
$startRow_rsCarreras = $pageNum_rsCarreras * $maxRows_rsCarreras;

mysql_select_db($database_conP12, $conP12);
$query_rsCarreras = "SELECT * FROM p12_carreras WHERE id_carrera <> 'No aplica' ORDER BY carrera ASC";
$query_limit_rsCarreras = sprintf("%s LIMIT %d, %d", $query_rsCarreras, $startRow_rsCarreras, $maxRows_rsCarreras);
$rsCarreras = mysql_query($query_limit_rsCarreras, $conP12) or die(mysql_error());
$row_rsCarreras = mysql_fetch_assoc($rsCarreras);

if (isset($_GET['totalRows_rsCarreras'])) {
  $totalRows_rsCarreras = $_GET['totalRows_rsCarreras'];
} else {
  $all_rsCarreras = mysql_query($query_rsCarreras);
  $totalRows_rsCarreras = mysql_num_rows($all_rsCarreras);
}
$totalPages_rsCarreras = ceil($totalRows_rsCarreras/$maxRows_rsCarreras)-1;
?>
<?php
$queryString_rsCarreras = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_rsCarreras") == false && 
        stristr($param, "totalRows_rsCarreras") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_rsCarreras = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_rsCarreras = sprintf("&totalRows_rsCarreras=%d%s", $totalRows_rsCarreras, $queryString_rsCarreras);
?>
<?php
if (!isset($_SESSION)) {
  session_start();
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Mantenimiento a BD &gt; Uso general &gt; Carreras</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <h2 class="center">Mostrando Carreras <?php echo ($startRow_rsCarreras + 1) ?> al <?php echo min($startRow_rsCarreras + $maxRows_rsCarreras, $totalRows_rsCarreras) ?> de <?php echo $totalRows_rsCarreras ?> </h2>
  <h3 class="center"><a href="car_add.php" class="estilo3">Agregar una carrera</a></h3>
  <table align="center" class="tabla_contactos">
    <tr>
    ...
    <?php do { ?>
      <tr>
        <td align="center"><?php echo $row_rsCarreras['id_carrera']; ?></td>
        <td><?php echo $row_rsCarreras['carrera']; ?></td>
        <td align="center"><a href="car_edit.php?id_carrera=<?php echo $row_rsCarreras['id_carrera']; ?>">Editar</a> | <a href="car_delete.php?id_carrera=<?php echo $row_rsCarreras['id_carrera']; ?>">Eliminar</a></td>
      </tr>
      <?php } while ($row_rsCarreras = mysql_fetch_assoc($rsCarreras)); ?>
  </table><br />
</div>
<!-- InstanceEndEditable -->
<div id="div_footer">
