...
$maxRows_rsNivelesDeAcceso = 10;
$pageNum_rsNivelesDeAcceso = 0;
if (isset($_GET['pageNum_rsNivelesDeAcceso'])) {
  $pageNum_rsNivelesDeAcceso = $_GET['pageNum_rsNivelesDeAcceso'];
}
$startRow_rsNivelesDeAcceso = $pageNum_rsNivelesDeAcceso * $maxRows_rsNivelesDeAcceso;
...
$query_limit_rsNivelesDeAcceso = sprintf("%s LIMIT %d, %d", $query_rsNivelesDeAcceso, $startRow_rsNivelesDeAcceso, $maxRows_rsNivelesDeAcceso);
$rsNivelesDeAcceso = mysql_query($query_limit_rsNivelesDeAcceso, $conP12) or die(mysql_error());
$row_rsNivelesDeAcceso = mysql_fetch_assoc($rsNivelesDeAcceso);

if (isset($_GET['totalRows_rsNivelesDeAcceso'])) {
  $totalRows_rsNivelesDeAcceso = $_GET['totalRows_rsNivelesDeAcceso'];
} else {
  $all_rsNivelesDeAcceso = mysql_query($query_rsNivelesDeAcceso);
  $totalRows_rsNivelesDeAcceso = mysql_num_rows($all_rsNivelesDeAcceso);
}
$totalPages_rsNivelesDeAcceso = ceil($totalRows_rsNivelesDeAcceso/$maxRows_rsNivelesDeAcceso)-1;
?>
<!DOCTYPE html>
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Mantenimiento a BD &gt; Niveles de acceso</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <h2 class="center">Mostrando
registros <?php echo ($startRow_rsNivelesDeAcceso + 1) ?> al <?php echo min($startRow_rsNivelesDeAcceso + $maxRows_rsNivelesDeAcceso, $totalRows_rsNivelesDeAcceso) ?> de <?php echo $totalRows_rsNivelesDeAcceso ?></h2>
  <h3 class="center"><a href="niv_acceso_add.php" class="estilo3">Agregar nuevo nivel de acceso</a></h3>
  <table width="400" align="center" class="tabla_contactos">
    <tr>
      <th scope="col">ID</th>
    ...
    <?php do { ?>
      <tr>
        <td align="center">&nbsp;</td>
        <td align="center"><?php echo $row_rsNivelesDeAcceso['nivel_de_acceso']; ?></td>
        <td align="center"><a href="niv_acceso_edit.php" class="estilo3">Editar</a> | <a href="niv_acceso_delete.php" class="estilo3">Eliminar</a></td>
      </tr>
      <?php } while ($row_rsNivelesDeAcceso = mysql_fetch_assoc($rsNivelesDeAcceso)); ?>
  </table><p class="center"><a href="index.php" class="button-link">Regresar</a></p>
</div>
<!-- InstanceEndEditable -->
<div id="div_footer">
