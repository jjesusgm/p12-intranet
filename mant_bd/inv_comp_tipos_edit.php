...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE inv_computo_tipos SET tipo_dispositivo=%s WHERE id_tipo_dispositivo=%s",
                       GetSQLValueString($_POST['fTipoDispositivo'], "text"),
                       GetSQLValueString($_POST['fIdTipoDispositivo'], "int"));

  mysql_select_db($database_conP12, $conP12);
  $Result1 = mysql_query($updateSQL, $conP12) or die(mysql_error());

  $updateGoTo = "inv_comp_tipos.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$colname_rsTipo = "-1";
if (isset($_GET['id_tipo_dispositivo'])) {
  $colname_rsTipo = $_GET['id_tipo_dispositivo'];
}
mysql_select_db($database_conP12, $conP12);
$query_rsTipo = sprintf("SELECT * FROM inv_computo_tipos WHERE id_tipo_dispositivo = %s", GetSQLValueString($colname_rsTipo, "int"));
$rsTipo = mysql_query($query_rsTipo, $conP12) or die(mysql_error());
...
<div id="div_hdr_path">&nbsp;Inicio &gt; Mantenimiento a BD &gt; Inventario &gt; Cómputo &gt; Tipos de dispositivo &gt; Editar</div>
<!-- InstanceEndEditable --><!-- InstanceBeginEditable name="Contenido" -->
<div id="div_contenido">
  <h2 class="center">Editando un tipo de dispositivo</h2>
  <form id="form1" name="form1" method="POST" action="<?php echo $editFormAction; ?>">
    <table width="400" align="center" class="tabla_usuario">
      <tr>
        <th colspan="2" align="left" scope="col">Registro a editar</th>
      </tr>
      <tr>
        <td align="right"><label for="fIdTipoDispositivo">ID</label></td>
        <td><input name="fIdTipoDispositivo" type="text" id="fIdTipoDispositivo" value="<?php echo $row_rsTipo['id_tipo_dispositivo']; ?>" size="5" maxlength="3" readonly /></td>
      </tr>
      <tr>
        <td align="right"><label for="fTipoDispositivo">Tipo*</label></td>
        <td><input name="fTipoDispositivo" type="text" id="fTipoDispositivo" value="<?php echo $row_rsTipo['tipo_dispositivo']; ?>" size="35" maxlength="30" /></td>
      </tr>
      <tr>
        <td colspan="2" align="center"><input type="submit" name="bActualizar" id="bActualizar" value="Actualizar" />
          <a href="inv_comp_tipos.php" class="button-link">Cancelar</a></td>
      </tr>
    </table>
